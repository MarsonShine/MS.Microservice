...
> 历史测量记录：本文描述合并前 `master` 上的实现和当时的测试结果，不描述 `codex/performance-aot` 合并后的代码。该分支已将 Core 的动态查询属性访问器迁到 Lab 示例，并为查询对象提供显式映射；Excel 也拆分了旧接口和独立的 AOT 项目。当前接口与 AOT 范围请看[迁移说明](Performance-Aot-Migration.md)。下表的“本轮结论”和末尾的“后续方向”均按测量当时的代码理解。

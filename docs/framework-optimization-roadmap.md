...
| HTTP 幂等的真实数据库并发验证 | [EF 幂等组件](../MS.Microservice.Idempotency/README.md)、Reference 的两套迁移和带键创建端点 | 在 PostgreSQL 下同时发送相同键/相同与不同正文，核对只有一次档案与 Outbox 效果，竞争失败方回滚后能重放赢家的 `201`；分别验证 SelfManaged 与 Wolverine |
...
| 多实例服务反复读取同一批数据，且能说明失效规则 | 共享缓存适配 | Core 已有接收显式 `JsonTypeInfo<T>` 的 `IDistributedCache` JSON 辅助方法；它只解决序列化元数据，不提供跨实例一致性或失效规则。选定真实缓存后端后，再验证跨实例读取、过期、并发回源、主动失效和后端不可用。 |
...
定时业务任务、Saga 与多租户等能力出现具体应用用例后，再列验收条件。消息 Outbox 的后台轮询只处理可靠消息，不代表已经有通用任务调度器。

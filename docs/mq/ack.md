...
在大多数时候，RabbitMQ 向发布者确认消息是在这些消息发布时的顺序是一样的（这应用在单通道上的发布的消息）。然而，发布者确认是异步触发的，且能确认单个消息或一组消息。发布确认消息的具体时间取决于消息的传递模式（持久化 vs 瞬时）以及被路由的队列消息的属性（见上）。那就是说不通的消息可能在不同的时间点确认。即确认各自的消息也能以不同的顺序到达。应用程序尽可能不应该依赖于确认的顺序。

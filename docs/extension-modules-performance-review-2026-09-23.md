...
> 历史测量记录：本文描述合并前 `master` 上的实现、局部基准和当时尚未完成的工作，不描述 `codex/performance-aot` 合并后的 AOT 状态。AI 的 JSON 元数据、Messaging 的合同序列化与 Wolverine 登记、Persistence 的软删除过滤和 SqlSugar JSON 入口已在该分支改用显式或生成的元数据；当前接口与验证范围请看[迁移说明](Performance-Aot-Migration.md)。末尾的“后续方向”是当时的计划，不能当作当前待办。

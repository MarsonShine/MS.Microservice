...
此类库属于 samples/AI，依赖 AI.Core、AI.Qwen 和通用 Core。命名空间由
MS.Microservice.AI.Core.Images 改为 MS.Microservice.Samples.EducationalImages，无兼容转发。
通用 AI 组件不依赖此场景。QuestionGeneration Harness 仍是共享可选包。

引用本项目后，先注册 AddMicroserviceAI(...).AddQwen()，再调用
services.AddImagePromptPipeline()；需要参考图编辑时额外调用 services.AddQwenEducationalImages()。
场景测试位于 test/MS.Microservice.Samples.EducationalImages.Tests。
所有密钥由宿主配置注入；仓库快速启动不需要 AI 账号。

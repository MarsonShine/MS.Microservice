...
`Http:Idempotency:Enabled` defaults to `false`. While it is off, `POST /api/v1/profiles` ignores `Idempotency-Key`, including malformed or repeated values, and follows the original create path. The host does not register the idempotency store or cleanup worker, and requests do not need the idempotency table. Readiness ignores the two known `AddHttpIdempotency` migrations in this mode, but still requires the other migrations for the selected provider. A repeated create request can still return `409` from the profile's business uniqueness rule.

When enabled, a valid `Idempotency-Key` lets a successful create save its `201` response with the profile and Outbox write. A retry can replay the original `Location` and body; a different request using the same key returns `409`. Invalid or conflicting business requests do not reserve the key. Unkeyed requests retain their existing behavior. Apply the selected provider's HTTP idempotency migration before enabling the feature; the host never creates the table automatically. See [创建档案时如何处理重复 HTTP 请求](idempotent-profile-create.md) for the transaction and cleanup behavior.

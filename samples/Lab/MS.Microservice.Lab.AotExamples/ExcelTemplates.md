...
```csharp
var builder = helper.OpenExcel(templateStream, rows, Books.Book); // Books 是生成上下文
builder.InitInsertRow(0, 1).InsertCellValue(1);
await builder.WriteAsync(output);
```

模板标题存在时按名称绑定，保留非连续列的位置；没有标题单元格时按映射声明顺序填入。插入行、复制样式、移动尾部行和写入流仍由原有 NPOI 代码完成。当前 builder 绑定 SG 生成的类型化 Write 操作，不缓存属性值，因此绑定后修改对象仍读取最新值。AOT 实现不使用 ReflectionDelegateFactory；原 Excel 包保持独立。
...
完整旧模板、委托工厂和颜色表保存在 `Legacy/Excel/*.cs.txt`。先前手写绑定例子位于 `Legacy/Excel/ManualMapping/TemplateMapping.cs`；当前 `Static/Excel/GeneratedBook.cs` 的生成结果同时用于常规工作簿和模板。生产实现把绑定结果复用于多行。验证覆盖样式、值类型、空标题、稀疏和重排标题、缺失列、空数据、尾行移动、管道输入以及整个 NPOI 颜色表。

...
public sealed class IdempotencyResponse
{
    ...
    public IdempotencyResponse(int statusCode, string contentType, ReadOnlySpan<byte> body, string? location = null)
    {
        ...
        if (string.IsNullOrWhiteSpace(contentType) || contentType.Length > 128 || contentType.Any(char.IsControl))
            throw new ArgumentException("Content type must contain 1..128 non-control characters.", nameof(contentType));

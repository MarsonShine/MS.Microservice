...
public sealed class MessageContractRegistryTests
{
    ...
    public void ContractRegistrationChecksFullRoutingKeyBytes(int characters, int version, bool valid)
    {
        ...
        var contract = MessageContract.For<Changed>(name, TestMessageJsonContext.Default.Changed, version);
        if (valid)
        {
    ...
    public void AsciiColumnBoundaryAndDottedNamesRemainValid()
    {
        ...
        var registry = new MessageContractRegistry([MessageContract.For<Changed>(name, TestMessageJsonContext.Default.Changed, int.MaxValue)]);
        Assert.Equal(name, registry.Get(typeof(Changed)).Name);
        Assert.Throws<ArgumentException>(() => new MessageContractRegistry([
            MessageContract.For<Changed>(name + "a", TestMessageJsonContext.Default.Changed, int.MaxValue)]));
    }
    ...
    public void ContractNameMustBeLosslesslyEncodableAsUtf8()
    {
        ...
        Assert.Throws<ArgumentException>(() => new MessageContractRegistry([MessageContract.For<Changed>(invalid, TestMessageJsonContext.Default.Changed)]));
    }

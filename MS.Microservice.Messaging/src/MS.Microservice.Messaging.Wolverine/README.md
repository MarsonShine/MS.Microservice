...
Register the provider with `builder.Host.UseWolverineMessaging<TContext>(topology, options, [WolverineMessageRegistration<TContext>.For<TEvent>()])`, listing every message contract once. The closed generic registrations make the adapter bridge visible to trimming and Native AOT. This registers one business context and native transaction integration; native storage auto-DDL is disabled. Map native envelopes in the application's provider-specific context, and apply the application's migration plus the native message-store schema through the separate migrator.

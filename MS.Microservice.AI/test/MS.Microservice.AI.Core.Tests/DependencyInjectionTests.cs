...
{
    [Fact]
    public void GenericCoreDoesNotReferenceScenarioOrLegacyCoreAssemblies()
    {
        var assembly = typeof(RoutingAIChatClient).Assembly;
        Assert.DoesNotContain(assembly.GetReferencedAssemblies(), reference =>
            reference.Name == "MS.Microservice.Core" || reference.Name!.Contains("EducationalImages"));
        Assert.DoesNotContain(assembly.GetTypes(), type => type.Namespace?.Contains(".Images") == true);
    }
    [Fact]

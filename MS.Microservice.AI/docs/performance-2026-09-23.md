...
上面的测量只涉及 DeepSeek 配置校验。`codex/performance-aot` 后续已让题目生成调用方登记 `JsonTypeInfo<T>`；`SystemTextJsonQuestionContract` 用登记的元数据进行序列化、反序列化和 Schema 生成，缺少元数据时会失败。它仍用 `Type` 查找已登记的元数据，但不再使用 `DefaultJsonTypeInfoResolver` 生成业务类型契约。整个 AI 模块及其外部依赖的原生发布兼容性仍需按目标宿主验证，详见[迁移说明](../../docs/Performance-Aot-Migration.md)。

...
├── ISceneGroupingAgent.cs               # 场景分组抽象接口
├── PlanGeneratorClient.cs               # 默认 LLM 规划实现（使用 IAIChatClient + scenario）
├── SceneGroupingAgent.cs                # 默认场景分组实现
...
│   ├── QwenSafePromptBuilder.cs              # Safe prompt（settingCues 上限 2 个）
│   ├── SentenceSemanticRulesProvider.cs      # 句子级语义规则
│   ├── SentenceImageContinuityPromptBuilder.cs # 连续场景 prompt 构建
...
`ISceneGroupingAgent` / `SceneGroupingAgent` 提供 LLM 驱动的句子分组能力，用于批量图片生成时保持组内视觉一致性：
...
**配置方式**：`AddImagePromptPipeline()` 自动注册 `ISceneGroupingAgent` 为 Singleton。
...
| `WordImagePromptPipeline` | (concrete) | Transient | 文本解析 → 视觉规划 → prompt 构建 |
| `ImageGenerationOrchestrator` | (concrete) | Transient | 一站式：文本 → prompt → 图片 |
